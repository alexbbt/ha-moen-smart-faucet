...
class MoenAPITester:
    ...
    def test_locations(self) -> bool:
        ...
        try:
            ...
            for location in locations:
                print(
                    f"  - {location.get('name', 'Unknown')} ({location.get('id', 'unknown')})"
                )
                print(f"    Address: {location.get('address', 'Unknown')}")
                print(
                    f"    City: {location.get('city', 'Unknown')}, {location.get('state', 'Unknown')}"
                )
            return True
        except Exception as e:
    ...
    def test_devices(self) -> bool:
        ...
        try:
            ...
            for device in devices:
                print(
                    f"  - {device.get('nickname', 'Unknown')} ({device.get('duid', 'unknown')})"
                )
    ...
    def test_water_control(self, client_id: str) -> bool:
        ...
        try:
            ...
            shadow = self.api.get_device_shadow(client_id)
            current_state = (
                shadow.get("state", {}).get("reported", {}).get("state", "unknown")
            )
            ...
            if devices:
                ...
                if client_id:
                    ...
                    if test_type in ["all", "water-control"]:
                        ...
                        test_water = (
                            input(
                                f"\nTest water control for {device.get('nickname', 'device')}? (y/n): "
                            )
                            .lower()
                            .strip()
                        )
...
Examples:
  ...
  python test_moen_api.py --water-control    # Test water control (with confirmation)
    ...
    parser.add_argument(
        "--auth-only", action="store_true", help="Test authentication only"
    )
    parser.add_argument(
        "--devices", action="store_true", help="Test device-related endpoints"
    )
    parser.add_argument(
        "--water-control",
        action="store_true",
    ...
    parser.add_argument(
        "--list-endpoints", action="store_true", help="List all available API endpoints"
    )
